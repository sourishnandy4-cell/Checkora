...
import { Play, Puzzle, Trophy, Flame, User, History, ArrowRight, Users, Copy, X } from 'lucide-react';
...
export const Home: React.FC = () => {
  ...
  const { peerId, isConnected, isConnecting, hostGame, joinGame, disconnect } = useMultiplayerStore();

  React.useEffect(() => {
    if (isConnected) navigate('/play');
    ...
    <div className="w-full h-full flex flex-col p-6 overflow-y-auto custom-scrollbar">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-6 flex flex-col gap-6">
          ...
          {/* Multiplayer Banner */}
          <div className="bg-bg-surface border border-bg-border p-6 rounded-sm relative overflow-hidden flex flex-col sm:flex-row items-center justify-between gap-6 group shadow-lg">
            <div className="absolute inset-0 bg-gradient-to-r from-accent-cyan/10 via-transparent to-transparent pointer-events-none" />
            <div className="z-10 text-center sm:text-left">
              <h2 className="font-serif-header text-xl font-bold mb-1 flex items-center justify-center sm:justify-start gap-2 text-text-primary">
                <Users size={20} className="text-accent-cyan" /> Play a Friend
              </h2>
              <p className="text-xs text-text-secondary max-w-sm leading-relaxed">
                Connect directly with a friend online! Generate a code to host, or enter their code to join.
              </p>
            </div>
            ...
            <button
              ...
              className="premium-btn py-3 px-8 text-xs uppercase font-mono-clock z-10 shrink-0 w-full sm:w-auto text-center"
            >
              Connect <ArrowRight size={16} className="inline ml-1" />
            </button>
          </div>
        </div>
